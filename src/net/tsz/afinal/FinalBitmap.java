...
public class FinalBitmap {
	...
	public FinalBitmap configLoadingAnimationDrawble(
			int loadingAnimationDrawbleResource) {
		mConfig.defaultDisplayConfig
				.setLoadingAnimationDrawable(loadingAnimationDrawbleResource);
		return this;
	}
	...
	public void display(View imageView, String uri,
			int loadingAnimationDrawableResource) {
		BitmapDisplayConfig displayConfig = configMap.get(String
				.valueOf("loadingAnimationDrawble"
						+ loadingAnimationDrawableResource));
		if (displayConfig == null) {
			displayConfig = getDisplayConfig();
			displayConfig
					.setLoadingAnimationDrawable(loadingAnimationDrawableResource);
			configMap.put(
					String.valueOf("loadingAnimationDrawble"
							+ loadingAnimationDrawableResource), displayConfig);
		}
	...
	private void doDisplay(View imageView, String uri,
		...
		if (bitmap != null) {
			...
			// 设置默认图片
			if (displayConfig.getLoadingAnimationDrawable() != -1) {
				AnimationDrawable loadingAd = (AnimationDrawable) mContext
						.getResources().getDrawable(
								displayConfig.getLoadingAnimationDrawable());
				if (loadingAd != null) {
					imageView
							.setOnFocusChangeListener(new AsyncOnFocusChangeListener(
		...
		if (imageView != null) {
			...
			if (drawable instanceof AsyncDrawable) {
				...
				if (l instanceof AsyncOnFocusChangeListener) {
					return ((AsyncOnFocusChangeListener) l)
							.getBitmapWorkerTask();
				}
			}
		}
	...
	private static class AsyncOnFocusChangeListener implements
			OnFocusChangeListener {
		private final WeakReference<BitmapLoadAndDisplayTask> bitmapWorkerTaskReference;
		private OnFocusChangeListener originOnFocusChangeListener;

		public AsyncOnFocusChangeListener(
				OnFocusChangeListener originOnFocusChangeListener,
				BitmapLoadAndDisplayTask bitmapWorkerTask) {
